...
describe( 'typeUtils mixin', () => {
	...
	describe( 'selectBestLanguage', () => {
		it( 'returns undefined when there are no options', () => {
			mw.language.getFallbackLanguageChain = jest.fn( () => [ 'en' ] );
			const allNames = [];
			expect( typeUtils.selectBestLanguage( allNames ) ).toBe( undefined );
		} );

		it( 'returns first option when fallback chain has no info', () => {
			mw.language.getFallbackLanguageChain = jest.fn( () => [] );
			const allNames = [
				{ langZid: 'Z1002', langIsoCode: 'en', rowId: 1 },
				{ langZid: 'Z1003', langIsoCode: 'es', rowId: 2 },
				{ langZid: 'Z1004', langIsoCode: 'fr', rowId: 3 }
			];
			const bestOne = { langZid: 'Z1002', langIsoCode: 'en', rowId: 1 };
			expect( typeUtils.selectBestLanguage( allNames ) ).toEqual( bestOne );
		} );

		it( 'returns preferred language when placed first', () => {
			mw.language.getFallbackLanguageChain = jest.fn( () => [ 'en' ] );
			const allNames = [
				{ langZid: 'Z1002', langIsoCode: 'en', rowId: 1 },
				{ langZid: 'Z1003', langIsoCode: 'es', rowId: 2 },
		...
		it( 'returns preferred language when placed last', () => {
			mw.language.getFallbackLanguageChain = jest.fn( () => [ 'fr', 'en' ] );
			const allNames = [
				{ langZid: 'Z1002', langIsoCode: 'en', rowId: 1 },
				{ langZid: 'Z1003', langIsoCode: 'es', rowId: 2 },
				{ langZid: 'Z1004', langIsoCode: 'fr', rowId: 3 }
			];
			const bestOne = { langZid: 'Z1004', langIsoCode: 'fr', rowId: 3 };
			expect( typeUtils.selectBestLanguage( allNames ) ).toEqual( bestOne );
		} );

		it( 'returns fallback when preferred language is not available', () => {
			mw.language.getFallbackLanguageChain = jest.fn( () => [ 'it', 'en' ] );
			const allNames = [
				{ langZid: 'Z1002', langIsoCode: 'en', rowId: 1 },
				{ langZid: 'Z1003', langIsoCode: 'es', rowId: 2 },
				{ langZid: 'Z1004', langIsoCode: 'fr', rowId: 3 }
