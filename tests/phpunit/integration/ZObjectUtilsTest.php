...
class ZObjectUtilsTest extends WikiLambdaIntegrationTestCase {
	 ...
	 *
	 * @dataProvider provideBenjaminPipe
	 * @covers ::benjaminToSimple
	 * @covers ::simpleToBenjamin
	 * @covers ::inferItemType
	 */
	public function testBenjaminPipe( $simple, $benjamin ) {
		$fromBenjamin = ZObjectUtils::benjaminToSimple( json_decode( $benjamin ) );
		$fromSimple = ZObjectUtils::simpleToBenjamin( json_decode( $simple ) );

		// Test benjamin to simple
		$this->assertSame(
			FormatJson::encode( FormatJson::parse( $simple )->value ),
			FormatJson::encode( $fromBenjamin )
		);

		// Test simple to benjamin
		$this->assertSame(
			FormatJson::encode( FormatJson::parse( $benjamin )->value ),
			FormatJson::encode( $fromSimple )
		);
	}

	public function provideBenjaminPipe() {
		return [
			'string' => [
				'"hola"',
				'"hola"'
			...
			'list with same zobjects' => [
				'[ { "Z1K1": "Z39", "Z39K1": "K1" }, { "Z1K1": "Z39", "Z39K1": "K2" } ]',
				'[ "Z39", { "Z1K1": "Z39", "Z39K1": "K1" }, { "Z1K1": "Z39", "Z39K1": "K2" } ]'
			],
			'list with different zobjects' => [
				'[ { "Z1K1": "Z39", "Z39K1": "K1" }, { "Z1K1": "Z99", "Z99K1": "wombat" } ]',
				'[ "Z1", { "Z1K1": "Z39", "Z39K1": "K1" }, { "Z1K1": "Z99", "Z99K1": "wombat" } ]',
			],
			'inner list with different zobjects' => [
				'{ "Z1K1": "Z401", "Z401K1": ['
					. ' { "Z1K1": "Z39", "Z39K1": "K1" },'
					. ' { "Z1K1": "Z99", "Z99K1": "wombat" } ] }',
				'{ "Z1K1": "Z401", "Z401K1": ['
					. ' "Z1",'
					. ' { "Z1K1": "Z39", "Z39K1": "K1" },'
					. ' { "Z1K1": "Z99", "Z99K1": "wombat" } ] }'
			...
			'list with list' => [
				'[ [ "dodo", "Z401" ] ]',
				'[ { "Z1K1": "Z7", "Z7K1": "Z881", "Z881K1": "Z1" }, [ "Z1", "dodo", "Z401" ] ]'
			],
			'list with list of strings' => [
				'[ [ "string", "wombat" ] ]',
				'[ { "Z1K1": "Z7", "Z7K1": "Z881", "Z881K1": "Z6" }, [ "Z6", "string", "wombat" ] ]'
			],
			'list with list and other element' => [
				'[ [ "dudeling", "pisco" ], "mambo" ]',
				'[ "Z1", [ "Z6", "dudeling", "pisco" ], "mambo" ]'
			],
			'list with two lists of strings' => [
				'[ [ "soup", "soup" ], [ "oh", "tasty", "soup" ] ]',
				'[ { "Z1K1": "Z7", "Z7K1": "Z881", "Z881K1": "Z6" },'
				. '[ "Z6", "soup", "soup" ],'
				. '[ "Z6", "oh", "tasty", "soup" ] ]'
			]
