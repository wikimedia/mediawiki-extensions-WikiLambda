	...
	getters: {
		...
		getStringifiedZObject: function ( state, getters ) {
			function stringifyZObject( zObject ) {
				if ( Array.isArray( zObject ) ) {
					zObject.forEach( stringifyZObject );
				} else if ( typeof zObject === 'object' ) {
					Object.keys( zObject ).forEach( function ( key ) {
						var stringLabel = getters.getStringifiedZObjectReference( getters.getZkeyLabels[ key ] || key ),
							value = zObject[ key ];

						delete zObject[ key ];
						zObject[ stringLabel ] = value;

						if ( typeof value === 'object' ) {
							stringifyZObject( zObject[ stringLabel ] );
						} else {
							zObject[ stringLabel ] = getters.getStringifiedZObjectReference(
								getters.getZkeyLabels[ value ] || value
							);
						}
					} );
				}
			...
			return stringifyZObject;
		},
		getStringifiedZObjectReference: function ( state, getters ) {
			return function ( str ) {
				var zKeyRegex = new RegExp( /Z[1-9]\d*(K[1-9]\d*)/ );

				if ( zKeyRegex.test( str ) ) {
					// Get the root
					var zId = str.match( /Z[1-9]\d*/ )[ 0 ];

					// Find the object
					var zObject = getters.getZkeys[ zId ];
					...
					// Find the argument
					var zArgument = zObject.Z2K2.Z8K1.filter( function ( arg ) {
						return arg.Z17K2 === str;
					} )[ 0 ];
					...
					while ( langStr === '' && langOptions.length > 0 ) {
						var langCode = langOptions.shift(),
							lang = null;

						for ( var index in zArgument.Z17K3.Z12K1 ) {
							var label = zArgument.Z17K3.Z12K1[ index ];

							if ( label.Z11K1 === langCode ) {
								lang = label;
								break;
							}
						}
					...
					if ( langStr === '' ) {
						langStr = zArgument.Z17K3.Z12K1[ 0 ].Z11K2;
					}
